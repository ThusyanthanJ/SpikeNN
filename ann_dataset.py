        ...
        if self.modality == 'RGB' or self.modality == 'RGBDiff':
            try:
                ...
                print('error loading image:', os.path.join(
                    self.root_path, "rgb", directory, self.image_tmpl.format(idx)))
                return [Image.open(os.path.join(self.root_path, "rgb", directory, self.image_tmpl.format(1))).convert('RGB')]

        elif self.modality == 'Flow':
            try:
                x_img = Image.open(os.path.join(
                    self.root_path, "flow/u", directory, self.image_tmpl.format(idx))).convert('L')
                y_img = Image.open(os.path.join(
                    self.root_path, "flow/v", directory, self.image_tmpl.format(idx))).convert('L')
            except Exception:
                print('error loading flow file:', os.path.join(
                    self.root_path, "flow/v", directory, self.image_tmpl.format(idx)))
                x_img = Image.open(os.path.join(
                    self.root_path, "flow/u", directory, self.image_tmpl.format(1))).convert('L')
                y_img = Image.open(os.path.join(
                    self.root_path, "flow/v", directory, self.image_tmpl.format(1))).convert('L')
            return [x_img, y_img]
