...
class TSN(nn.Module):
    ...
    TSN Configurations:
        ...
        if self.modality == 'Flow':
            print("Converting the  model to a flow init model")
            ...
            print("Converting the model to RGB+Flow init model")
    ...
    def _prepare_base_model(self, base_model):
        ...
        if 'resnet' in base_model or 'vgg' in base_model or 'squeezenet1_1' in base_model:
            self.base_model = pretrainedmodels.__dict__[
                base_model](num_classes=1000, pretrained='imagenet')
    ...
    def _construct_rgbflow_model(self, base_model):
        ...
        filter_conv2d = filter(lambda x: isinstance(
            modules[x], nn.Conv2d), list(range(len(modules))))
        ...
        new_kernels = torch.cat((params[0].data.mean(dim=1, keepdim=True).expand(new_kernel_size).contiguous(
            # NOTE: Concatanating might be other way around. Check it!
        ), params[0].data), 1)
        new_kernel_size = kernel_size[:1] + \
            (3 + 2 * self.new_length,) + kernel_size[2:]
