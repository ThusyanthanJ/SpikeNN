if __name__ == '__main__':
    ...
    for epoch in range(args.train_num_epochs):
        ...
        snn_train_loader = Loader(
            train_ds, args, device, distributed=args.train_distributed, batch_size=args.train_batch_size, loader_type="train_ds", experiment_set="3")
            ...
            for (events, labels, folder_names), (i, (data, label)) in zip(snn_train_loader, enumerate(ann_data_loader)):
                ...
                try:
                    ...
                    for j in range(len(data)):
                        # indices = np.array([1, 19, 20, 9, 23]) # top 5
                        # indices = np.array([14, 18, 15, 17, 16])  # random 5
