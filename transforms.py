...
class GroupSpatialElasticDisplacement(object):
    ...
    def __call__(self, img_group):
        ...
        if v < 0.5:
            ...
            displacement_map = np.random.rand(
                image_h, image_w, 2) * 2 * self.displacement - self.displacement
            ...
            displacement_map_rows = displacement_map[..., 0] + np.tile(
                np.arange(image_h), (image_w, 1)).T.astype('int32')
            displacement_map_rows = np.clip(
                displacement_map_rows, 0, image_h - 1)

            displacement_map_cols = displacement_map[..., 1] + np.tile(
                np.arange(image_w), (image_h, 1)).astype('int32')
            displacement_map_cols = np.clip(
                displacement_map_cols, 0, image_w - 1)
            ret_img_group = [Image.fromarray(np.asarray(img)[(displacement_map_rows.flatten(
            ), displacement_map_cols.flatten())].reshape(np.asarray(img).shape)) for img in img_group]
...
class GroupMultiScaleResize(object):
    ...
    def __call__(self, img_group):
        im_size = img_group[0].size
        self.resize_const = random.uniform(
            1.0 - self.scale, 1.0 + self.scale)  # Aplly random resize constant
        resize_img_group = [img.resize(
            (int(im_size[0]*self.resize_const), int(im_size[1]*self.resize_const))) for img in img_group]
...
class GroupMultiScaleRotate(object):
    ...
    def __call__(self, img_group):
        im_size = img_group[0].size
        # Aplly random rotation angle
        self.rotate_angle = random.randint(-self.degree, self.degree)
        ret_img_group = [img.rotate(
            self.rotate_angle, resample=self.interpolation) for img in img_group]
    ...
    def __call__(self, img_group):
        if self.isRGBFlow:
            ...
            for x in img_group:
                if x.mode == 'L':
                    if stacked_array.size == 0:
                        ...
                        stacked_array = np.concatenate(
                            [stacked_array, np.expand_dims(x, 2)], axis=2)
                elif x.mode == 'RGB':
                    if self.roll:
                        stacked_array = np.concatenate(
                            [stacked_array, np.array(x)[:, :, ::-1]], axis=2)
                    else:
                        stacked_array = np.concatenate(
                            [stacked_array, np.array(x)], axis=2)
            ...
            if img_group[0].mode == 'L':
                ...
                if self.roll:
                    asd = np.concatenate([np.array(x)[:, :, ::-1]
